...
class Topology:
    ...
    def _link_atomics(self):
        """link atoms, bonds etc. properties of self (=top) to the merged molecule.

        Call this any time a large change is made to the merged moleculetype
        that re-assigns a property and not just modifies parts of it in place.
        """
        self.merged_molecule = self.moleculetypes[self.merged_moleculetype]
        self.atoms = self.merged_molecule.atoms
        self.bonds = self.merged_molecule.bonds
        self.angles = self.merged_molecule.angles
        self.exclusions = self.merged_molecule.exclusions
        self.settles = self.merged_molecule.settles
        self.proper_dihedrals = self.merged_molecule.proper_dihedrals
        self.improper_dihedrals = self.merged_molecule.improper_dihedrals
        self.pairs = self.merged_molecule.pairs
        self.position_restraints = self.merged_molecule.position_restraints
        self.dihedral_restraints = self.merged_molecule.dihedral_restraints
        self.radicals = self.merged_molecule.radicals
        self.nrexcl = self.merged_molecule.nrexcl

    def _extract_mergable_molecules(self) -> dict[str, int]:
        """Extract all molecules that are to be merged into one moleculetype.
        And replaces the moleculetypes with a single moleculetype with the name
        """
        included_molecules = {}
        new_molecules = []
        ...
        self.merged_moleculetype = "-".join(included_molecules.keys())
        new_molecules.insert(0, (self.merged_moleculetype, "1"))  # The merged moleculetype is inserted at the start of the list to comply with previous code behaviour
        logger.debug(
            f"Merging the following molecules into a single moleculetype called {self.merged_moleculetype} and making their multiples explicit:"
        )
    ...
    def _merge_moleculetypes(
        ...
        merged_moleculetype = MoleculeType(
            MoleculeTypeHeader(name=self.merged_moleculetype, nrexcl=nrexcl),
            merged_atomics,
            radicals,
        )
        self.moleculetypes[self.merged_moleculetype] = merged_moleculetype
    ...
    def _update_dict(self):
        ...
        for name, moleculetype in self.moleculetypes.items():
            ...
            if name == self.merged_moleculetype:
    ...
    def __str__(self) -> str:
        ...
        merged_moleculetype = self.moleculetypes.get(self.merged_moleculetype)
        text = (
            textwrap.dedent(f"Topology with the following molecules: \n{molecules}\n\n")
            + textwrap.dedent(f"With {merged_moleculetype}")
